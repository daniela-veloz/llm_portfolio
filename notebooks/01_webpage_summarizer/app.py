...
class RateLimiter:
    ...
    def check_rate_limit(self, ip_address):
        ...
        if current_time - data.get('last_request', 0) < self.cooldown_seconds:
            ...
            return False, f"⏰ Please wait {remaining_cooldown} seconds between requests", self._get_usage_stats(
        ...
        if len(hourly_requests) >= self.hourly_limit:
            ...
            return False, f"🚫 Hourly limit reached ({self.hourly_limit} requests/hour). Try again in {next_reset} minutes.", self._get_usage_stats(
                data['requests'], current_time)

        # Check daily limit
        if len(data['requests']) >= self.daily_limit:
            next_reset = int((min(data['requests']) + 24 * 3600 - current_time) / 3600)
            return False, f"🚫 Daily limit reached ({self.daily_limit} requests/day). Try again in {next_reset} hours.", self._get_usage_stats(
                data['requests'], current_time)
